 ...
 public:
  ...
  void receiveCommand(const std::string& cmd, JsonObject& data,
    ...
    if (cmd == "CreateEntity") {
      ...
      std::cout << "Made it to the back end\n";
      // model.stop();
      ...
      bool isCreated = std::filesystem::create_directory(folderName);

      if (!isCreated && !std::filesystem::exists(folderName)) {
        std::cerr << "Error: Failed to create directory '" << folderName << "'"
                  << std::endl;
      }

      std::string filename =
          folderName + "/drone_data_" + timestamp.str() + ".csv";

      DataManager::getInstance()->exportDataToCSV(filename);

      std::cout << "Data exported to " << filename << std::endl;
      JsonObject details;
      details["filename"] = filename;
      sendEventToView("Created File", details);
...
class TransitWebServer : public WebServerBase, public IController {
 public:
  TransitWebServer(int port = 8081, const std::string& webDir = ".")
      : WebServerBase(port, webDir), model(*this), alive_(true) {}
  void addEntity(const IEntity& entity) {
